...
//	}
	...
	public boolean updateAppointmentsStatus( UpdateAppointmentDTO appointment) {
		...
		// Check if user has address, but don't block approval if they don't
		boolean hasAddress = addressDao.findUserAddress(user.getId()) != null;
		log.info("User {} has address: {}", user.getId(), hasAddress);
		
		// Proceed with status update regardless of address
		...
		if (status.equalsIgnoreCase("APPROVED")) {
			...
			int availableBlood = bloodInventoryService.findByBloodGroupAndBagSize(appointmentById.getBloodGroup(),
					appointmentById.getBagSize());
			log.info("Available blood for {} {}ml: {}", appointmentById.getBloodGroup(), appointmentById.getBagSize(), availableBlood);
			log.info("Required blood: {}", appointmentById.getBagQuantity());
			
			if (updatedsts != 1 || availableBlood < appointmentById.getBagQuantity()) {
				log.error("Cannot approve appointment - insufficient blood or update failed. Available: {}, Required: {}, Update result: {}", 
					availableBlood, appointmentById.getBagQuantity(), updatedsts);
				return false;
			}
			...
			messageBody = "hello <p><font color=blue>" + user.getFirstName() + "</font></p>" +
